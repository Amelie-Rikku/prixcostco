...
- **Prix unitaire calculé** : affiché automatiquement à partir du prix et du format (ex. 3,99 $ / 500g → 0,80 $/100g)
...
- **Rafraîchissement à la demande** : un bouton "Mettre à jour les promos" déclenche l'appel à Flipp
...
- **Photo d'étiquette (OCR)** : capture le nom du produit, le prix et le poids/format depuis l'étiquette en rayon
- Si le produit n'existe pas encore dans la base : formulaire d'ajout pré-rempli avec les données captées
...
- **Vue résultat** : pour chaque produit, affichage du prix dans chaque magasin (avec format) et indicateur visuel du moins cher
- **Produits introuvables** : affichés clairement avec contrôle d'ajout direct depuis la vue résultat

### 4.5 Promotions

- Les promos sont distinguées visuellement des prix réguliers
- Chaque promo affiche sa date de validité
...
- Historique des prix et analyse de tendances
...
| Hébergement | Vercel | Déploiement automatique via GitHub push, gratuit |
...
| OCR étiquette | Google Cloud Vision ou Tesseract.js | Extraction nom + prix + format |
...
shopping_lists
  id, created_at, items[] (texte brut + product_id résolu)
```

### Flux de données

```
Flipp API ──→ Supabase (store_prices)
                    ↑
...
- Les promotions se mettent à jour en un tap et disparaissent quand elles expirent
